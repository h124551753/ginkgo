package com.molmc.ginkgo.stepcount.service;
...
import com.molmc.ginkgo.stepcount.accelerometer.StepCount;
import com.molmc.ginkgo.stepcount.accelerometer.StepValuePassListener;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initTodayData() {
        ...
        //获取当天的数据，用于展示
        List<StepDataEntity> list = DBDataSource.getInstance(this).getStepDataToday();
        if (list.size() == 0 || list.isEmpty()) {
            CURRENT_STEP = 0;
        } else if (list.size() == 1) {
            Log.v(TAG, "StepDataEntity=" + list.get(0).toString());
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void updateNotification() {
        //设置点击跳转
        Intent hangIntent = new Intent(this, StepPanelActivity.class);
    ...
    private void save() {
        ...
        List<StepDataEntity> list = DBDataSource.getInstance(this).getStepDataToday();
        if (list.size() == 0 || list.isEmpty()) {
            StepDataEntity data = new StepDataEntity();
            data.setToday(CURRENT_DATE);
            data.setStep(tempStep + "");
            DBDataSource.getInstance(this).saveStepDataEntity(data);
        } else if (list.size() == 1) {
            StepDataEntity data = list.get(0);
            data.setStep(tempStep + "");
            DBDataSource.getInstance(this).saveStepDataEntity(data);
        } else {
        }
